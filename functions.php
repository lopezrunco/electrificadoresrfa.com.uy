...
function enqueue_styles() {
    ...
    wp_enqueue_style( 'rfa-style', get_stylesheet_uri() );

    // Enqueue the custom WooCommerce stylesheet
    wp_enqueue_style( 'custom-woocommerce', get_template_directory_uri() . '/custom-woocommerce.css', array('rfa-style') );
}
...
function add_wapp_info_button() {
    ...
    $wapp_url = "https://api.whatsapp.com/send/?phone={$phone}&text=Hola%2C+quisiera+consultar+acerca+de:+{$encoded_title}&type=phone_number&app_absent=0";
    ?>

    <a href="<?php echo esc_url( $wapp_url ); ?>" target="_blank" rel="noreferrer" class="btn btn-ask-price" >
        <i class="fa-brands fa-whatsapp"></i> Hacer una consulta
    </a>
    <?php
}

// Change default US dollar symbol.
